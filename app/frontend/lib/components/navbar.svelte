      ...
      <div class="flex items-center space-x-4">
          ...
          {#if currentUser}
            <DropdownMenu.Root>
              ...
              <DropdownMenu.Content class="w-64" align="end">
                ...
                <DropdownMenu.Group>
                  <DropdownMenu.Item
                    onclick={() => router.visit("/dashboard")} onkeydown={(e) => (e.key === 'Enter' || e.key === ' ') && (() => router.visit("/dashboard"))}
                  ...
                  <DropdownMenu.Item
                    onclick={() => router.visit("/profile")} onkeydown={(e) => (e.key === 'Enter' || e.key === ' ') && (() => router.visit("/profile"))}
                    class="cursor-pointer"
                  >
                ...
                <DropdownMenu.Separator />
                <DropdownMenu.Item
                  onclick={handleLogout} onkeydown={(e) => (e.key === 'Enter' || e.key === ' ') && (handleLogout)}
        ...
        <div class="flex md:hidden">
          <Button
            ...
            size="sm"
            class="h-9 w-9 px-0"
            onclick={toggleMobileMenu} onkeydown={(e) => (e.key === 'Enter' || e.key === ' ') && (toggleMobileMenu)}
          >
            {#if mobileMenuOpen}
              <X class="h-5 w-5" />
            {:else}
  ...
  {#if mobileMenuOpen}
    <div class="border-t bg-background md:hidden">
      <div class="mx-auto max-w-7xl space-y-4 px-4 py-4 sm:px-6 lg:px-8">
        {#if currentUser}
          <!-- User Info -->
          <Link href="/profile" onclick={closeMobileMenu} onkeydown={(e) => (e.key === 'Enter' || e.key === ' ') && (closeMobileMenu)}>
            <div
          ...
          <nav class="space-y-1">
            <Link
              href="/dashboard"
              onclick={closeMobileMenu} onkeydown={(e) => (e.key === 'Enter' || e.key === ' ') && (closeMobileMenu)}
        ...
        {:else}
          ...
          <!-- Auth Buttons -->
          <div class="space-y-2">
            <Link href={loginPath()} onclick={closeMobileMenu} onkeydown={(e) => (e.key === 'Enter' || e.key === ' ') && (closeMobileMenu)} class="block">
              <Button variant="outline" class="w-full justify-center">
                Log in
              </Button>
            </Link>
            <Link href={signupPath()} onclick={closeMobileMenu} onkeydown={(e) => (e.key === 'Enter' || e.key === ' ') && (closeMobileMenu)} class="block">
